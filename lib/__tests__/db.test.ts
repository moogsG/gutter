...
describe("Database operations", () => {
  ...
  describe("journal_entries CRUD", () => {
    ...
    it("retrieves entries by date", () => {
      ...
      const entries = db
        .prepare("SELECT * FROM journal_entries WHERE date = ? ORDER BY sort_order")
        .all("2026-03-11");
    ...
    it("updates entry status", () => {
      ...
      db.prepare("UPDATE journal_entries SET status = ? WHERE id = ?").run("complete", id);
  ...
  describe("collections", () => {
    ...
    it("links entry to collection", () => {
      ...
      db.prepare("INSERT INTO collections (id, title) VALUES (?, ?)").run(collId, "Projects");
      db.prepare(
  ...
  describe("migration", () => {
    it("migrates entry to another date", () => {
      ...
      db.prepare("UPDATE journal_entries SET status = ?, migrated_to = ? WHERE id = ?")
      ...
      const src = db.prepare("SELECT * FROM journal_entries WHERE id = ?").get(srcId);
      const dest = db.prepare("SELECT * FROM journal_entries WHERE id = ?").get(destId);
