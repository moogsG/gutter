...
export const RETRY_ATTEMPTS = parseInt(
	process.env.JIRA_RETRY_ATTEMPTS || "3",
	10,
);
export const RETRY_DELAY_MS = parseInt(
	process.env.JIRA_RETRY_DELAY_MS || "1000",
	10,
);
export const CACHE_DURATION_MS = parseInt(
	process.env.JIRA_CACHE_DURATION_MS || "300000",
	10,
); // 5 min
...
export async function updateIssueStatus(
	...
	if (!transition) {
		throw new Error(
			`No valid transition to status "${statusName}" for issue ${issueKey}`,
		);
	}
...
export function getJiraStatus() {
	return {
		...
		cacheValid: jiraCache.lastSync
			? Date.now() - jiraCache.lastSync < CACHE_DURATION_MS
			: false,
