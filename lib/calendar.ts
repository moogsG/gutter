...
export const RETRY_ATTEMPTS = parseInt(process.env.CALENDAR_RETRY_ATTEMPTS || "3", 10);
export const RETRY_DELAY_MS = parseInt(process.env.CALENDAR_RETRY_DELAY_MS || "1000", 10);
export const CACHE_DURATION_MS = parseInt(process.env.CALENDAR_CACHE_DURATION_MS || "300000", 10);
...
export async function executeWithRetry(cmd: string, attempts = RETRY_ATTEMPTS): Promise<string> {
...
export async function createCalendarEvent(params: CalendarEventParams): Promise<{
  ...
  const { summary, date, startTime, endTime, allDay, calendar, location, description } = params;
