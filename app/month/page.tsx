...
import type { MeetingPrep, CalendarEvent } from "@/types";
...
function buildCalendarGrid(year: number, month: number, events: CalendarEvent[]): CalendarDay[][] {
...
const calColorClasses: Record<string, { bg: string; border: string; text: string; dot: string }> = {
  "cal-gradient": { bg: "bg-cal-gradient/15", border: "border-cal-gradient/50", text: "text-cal-gradient", dot: "bg-cal-gradient" },
  "cal-family": { bg: "bg-cal-family/15", border: "border-cal-family/50", text: "text-cal-family", dot: "bg-cal-family" },
  "cal-home": { bg: "bg-cal-home/15", border: "border-cal-home/50", text: "text-cal-home", dot: "bg-cal-home" },
  "cal-jw": { bg: "bg-cal-jw/15", border: "border-cal-jw/50", text: "text-cal-jw", dot: "bg-cal-jw" },
  "cal-school": { bg: "bg-cal-school/15", border: "border-cal-school/50", text: "text-cal-school", dot: "bg-cal-school" },
  "cal-birthdays": { bg: "bg-cal-birthdays/15", border: "border-cal-birthdays/50", text: "text-cal-birthdays", dot: "bg-cal-birthdays" },
  "cal-holidays": { bg: "bg-cal-holidays/15", border: "border-cal-holidays/50", text: "text-cal-holidays", dot: "bg-cal-holidays" },
};
  ...
  return (
    ...
    >
      <span className="flex items-center gap-1">
        {startTime && <span className="text-muted-foreground shrink-0">{startTime}</span>}
...
function DayEventList({
  ...
  return (
    <div ref={containerRef} className="hidden sm:flex flex-col gap-0.5 overflow-hidden flex-1 min-h-0">
      {shown.map((event) => (
        <EventPill
          key={event.id}
          event={event}
...
export default function MonthlyLogPage() {
  ...
  const [selectedMeeting, setSelectedMeeting] = useState<MeetingPrep | null>(null);
  ...
  const weeks = useMemo(
    () => buildCalendarGrid(currentMonth.getFullYear(), currentMonth.getMonth(), calendarEvents),
  ...
  const getMeetingPrep = (eventId: string, eventDate?: string): MeetingPrep | null => {
    if (eventDate) {
    ...
    return meetingPrepData?.meetings.find((m) => m.eventId === eventId && !eventDate) || null;
  };
    ...
    <>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-3 sm:px-6 py-2 sm:py-3 border-b border-border/50">
          <Button variant="ghost" size="sm" onClick={handlePrevMonth} className="w-8 h-8 p-0">
            <ChevronLeft className="w-4 h-4" />
          </Button>
          <h2 className="text-base sm:text-xl font-bold text-foreground">{monthName}</h2>
          <Button variant="ghost" size="sm" onClick={handleNextMonth} className="w-8 h-8 p-0">
        ...
        {isLoading ? (
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div
              ...
              style={{ display: "grid", gridTemplateRows: `repeat(${weeks.length}, minmax(60px, 1fr))` }}
            >
              {weeks.map((week, wi) => (
                  ...
                  {week.map((day) => (
                    <div
                      ...
                      className={cn(
                        ...
                        selectedDay?.dateStr === day.dateStr && "bg-primary/10 ring-1 ring-inset ring-primary/30",
                      ...
                      <div className="flex items-center justify-between mb-0.5 shrink-0">
                        ...
                        {day.events.length > 0 && (
                          <span className="sm:hidden flex items-center gap-0.5">
                            ...
                            {[...new Set(day.events.map((e) => e.calendar))].slice(0, 3).map((cal) => (
                              <span key={cal} className={cn("w-1.5 h-1.5 rounded-full", getCalClasses(cal).dot)} />
                            ))}
                          </span>
                        )}
                      </div>
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                              ...
                              <Badge
                                variant="outline"
                                className={cn("text-[10px]", `border-${getCalendarColorToken(event.calendar)}/30`, colors.text)}
                              >
                                {event.calendar}
                              </Badge>
                              {hasPrep && (
                              ...
                              {hasTranscript && (
                                <Badge variant="outline" className="text-[10px] border-chart-2/40 text-chart-2 gap-1">
