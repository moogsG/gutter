...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const entries = db.prepare(query).all(...params) as Array<Omit<FutureLogEntry, 'migrated'> & { migrated: number }>;
    ...
    return NextResponse.json({ error: "Failed to fetch future log" }, { status: 500 });
  }
}
