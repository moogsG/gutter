...
export async function POST(request: Request) {
  try {
    ...
    if (!eventId || !title || !time) {
      return Response.json({ error: 'Missing eventId, title, or time' }, { status: 400 });
    }

    const db = getDb();
    const now = new Date().toISOString();
    const occurrenceDate = time ? new Date(time).toISOString().split('T')[0] : new Date().toISOString().split('T')[0];

    // Upsert meeting prep row
    const existing = db.prepare('SELECT id FROM meeting_prep WHERE event_id = ? AND occurrence_date = ?').get(eventId, occurrenceDate) as any;
    ...
    if (existing) {
      ...
      db.prepare(
        `INSERT INTO meeting_prep (id, event_id, title, time, calendar, occurrence_date, prep_status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).run(id, eventId, title, time, calendar || '', occurrenceDate, 'preparing', now, now);
    }

    // Fire and forget — generate prep in background, update DB when done
    (async () => {
      try {
        const prepNotes = await generateMeetingPrep(title, time, calendar, context);
        ...
        console.error(`[meeting-prep] Failed to generate prep for "${title}":`, err.message);
        db.prepare(
          'UPDATE meeting_prep SET prep_status = ?, updated_at = ? WHERE id = ?'
