...
export async function POST(request: Request) {
  try {
    ...
    if (!eventId || !transcript) {
      return Response.json({ error: 'Missing eventId or transcript' }, { status: 400 });
    }
    ...
    if (existing) {
      ...
      db.prepare(
        `INSERT INTO meeting_prep (id, event_id, title, time, calendar, occurrence_date, prep_status, transcript, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).run(id, eventId, title || '', time || '', calendar || '', occurrenceDate, 'none', transcript, now, now);
    }
    ...
    try {
      ...
      execAsync(
        ...
        { timeout: 120_000 }
      ).catch((err: Error) => console.error('Transcript summary request failed:', err.message));
    } catch (err) {
