	...
	try {
		...
		const meetings = uniqueEvents.map((event: any, index: number) => {
			...
			if (prep?.action_items) {
				try {
					actionItems = JSON.parse(prep.action_items);
				} catch {}
			}
		...
		const includedKeys = new Set(
			meetings.map((m: any) => `${m.eventId}::${m.occurrenceDate}`),
		);

		for (const prep of allPrepRows) {
			const prepDate =
				prep.occurrence_date || (prep.time ? prep.time.split("T")[0] : "");
