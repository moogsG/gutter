...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const validation = validateJournalEntry({
      content: text,
      tags: ["health-log", "health-log:prep", "health-cut", "health-cut:prep-lock"],
    });

    if (!validation.valid) {
      await logValidationFailure(req, "/api/health-cut/prep-lock", {
    ...
    const sanitizedTags = validation.sanitized?.tags || ["health-log", "health-log:prep", "health-cut", "health-cut:prep-lock"];
    const db = getDb();
    ...
    }).catch((error) => console.error("[vector-store] prep lock upsert failed:", error));
    ...
    return handleApiError("create health prep lock", error);
  }
}
