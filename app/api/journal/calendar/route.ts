...
export async function POST(req: NextRequest) {
  try {
    ...
    const { summary, date, startTime, endTime, allDay, calendar, location, description } = body;
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    status: {
      ...
      lastSuccess: calendarCache.lastSuccess,
      lastError: calendarCache.lastError,
      timeSinceLastSync: calendarCache.lastSync ? Date.now() - calendarCache.lastSync : null,
    },
  });
}
